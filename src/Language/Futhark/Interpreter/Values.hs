...
data Value m
  ...
  | -- An external value
    ValueExt (Maybe (Value m)) FFI.GenFFIValue
...
instance Show (Value m) where
  ...
  show (ValueExt v _) = unwords ["ValueExt", show v, "_"]
...
prettyValueWith pprPrim = pprPrec 0
  where
    ...
    pprPrec _ (ValueExt Nothing _) = "ex"
    pprPrec p (ValueExt (Just v) _) = "ex" <> "(" <> pprPrec p v <> ")"
