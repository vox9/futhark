...
class Monad m => MonadUIDSource r m where
  getUID :: m (UID p r)
  getUIDs :: r -> m [UID p r]

instance (Monad m, Bounded r, Enum r) => MonadUIDSource r (UIDSourceT r m) where
  getUID = UIDSourceT $ state nextUID
  getUIDs n = mapM (const getUID) [minBound .. pred n]
