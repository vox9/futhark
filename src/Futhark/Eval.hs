...
import Language.Futhark.Interpreter.FFI.FutharkServer.Server (FutharkServer (efi), start')
import Language.Futhark.Interpreter.FFI.Generic (FFIValue)
import Language.Futhark.Interpreter.FFI.Generic qualified as FFI
import Language.Futhark.Interpreter.FFI.UIDs (runUIDSourceT, UIDSource, MonadUIDSource)
...
newtype InterpreterState = InterpreterState (VNameSource, T.Env, I.Ctx, Maybe FutharkServer, IORef UIDSource)
...
runExpr ::
  ...
  T.Text ->
  m (Doc AnsiStyle)
runExpr (InterpreterState (src, env, ctx, s, r)) str = do
  ...
  pval <- runInterpreterNoBreak r s $ I.interpretExp ctx fexp
  case (pval, s) of
    (Left err, _) -> do
      abort $ I.prettyInterpreterError err
    (Right v, Just _) -> do
      uis <- liftIO $ readIORef r
      (v', _) <- runUIDSourceT (realizeAll v) uis
      pure $ I.prettyValue v' <> hardline
    (Right val, Nothing) -> do
      pure $ I.prettyValue val <> hardline

realizeAll :: (MonadIO m, MonadUIDSource m) => I.Value -> m I.Value
realizeAll (IV.ValueArray s' a) = IV.ValueArray s' <$> traverse realizeAll a
realizeAll (IV.ValueRecord m) = IV.ValueRecord . M.fromList <$> mapM (\(k, v) -> (k,) <$> realizeAll v) (M.toList m)
realizeAll (IV.ValueSum s' v fs) = IV.ValueSum s' v <$> mapM realizeAll fs
realizeAll (IV.ValueExt e) = FFI.fetch e
realizeAll v = pure v
...
newFutharkiState cfg vfs = runExceptT $ do
  ...
  pure $ InterpreterState (src, tenv, ienv, s, r)
  where
...
runInterpreterNoBreak r (Just s) m = runF m (pure . Right) intOp
  where
    ...
    intOp (I.ExtOpCall n ps c) = do
      uis <- liftIO $ readIORef r
      (v, uis') <- runUIDSourceT (FFI.call (efi s) n $ map sortPs ps) uis
      liftIO $ writeIORef r uis'
      c $ IV.ValueExt v
    intOp (I.ExtOpPush v c) = do
      uis <- liftIO $ readIORef r
      (v', uis') <- runUIDSourceT (FFI.push v) uis
      liftIO $ writeIORef r uis'
      c $ v'
    intOp (I.ExtOpFetch e c) = do
      uis <- liftIO $ readIORef r
      (v, uis') <- runUIDSourceT (FFI.fetch e) uis
      liftIO $ writeIORef r uis'
      c v

runInterpreterNoBreak _ _ _ = error "TODO (98quoijwda) NO SERVER"

sortPs :: I.Value -> Either I.Value (FFIValue I.Value)
sortPs (IV.ValueExt e) = Right e
sortPs v = Left v
