...
interpret config fp = do
  ...
  case I.interpretFunction ienv (qualLeaf fname) inps of
    Left err -> do
      ...
      exitFailure
    Right run -> do
      run' <- runInterpreter' call realize run
      case run' of
        Left err -> do
          hPrint stderr err
          exitFailure
...
newFutharkiState ::
  InterpreterConfig ->
  FilePath ->
  IO (Either (Doc AnsiStyle) ([S.FutharkServer], T.Env, I.Ctx))
newFutharkiState cfg file = runExceptT $ do
  ...
  let loadImport ctx =
        badOnLeft I.prettyInterpreterError
          <=< runInterpreter' (const $ const $ error "TODO: Not needed? (9y8ud)") (const $ error "TODO: Not needed? (12e0u8oi)") . I.interpretImport ctx
