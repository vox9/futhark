...
import Futhark.AutoImp qualified as I
import Data.Foldable (foldlM)
import Futhark.Extras (extras)
import Language.Futhark.Syntax (ImportName(ImportName))
...
readProgramFile extra_eps path = do
  (w, i, ns) <- readProgramFiles extra_eps [path]
  let (i', ns') = either (error . T.unpack) id $ addExtras i ns
  pure (w, i', ns')

addExtras ::
  Imports ->
  VNameSource ->
  Either T.Text (Imports, VNameSource)
addExtras im ns = foldlM addExtras' ([], ns) im
  where
    addExtras' :: (Imports, VNameSource) -> (ImportName, FileModule) -> Either T.Text (Imports, VNameSource)
    addExtras' (i, ns') ni@(_, fm) = I.implement (extras $ fileProg fm) (i ++ [ni]) ns'
